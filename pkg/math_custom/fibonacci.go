...
func (s *FibonacciService) GetSlice(offset, limit uint64) []uint64 {
	return s.calcFibonacciSlice(CountFibonacci, offset, limit, s.cache)
}

type countFibonacciFunc func(uint64) uint64

func (s *FibonacciService) calcFibonacciSlice(countFibonacci countFibonacciFunc, x, y uint64, cache *cache.Store) []uint64 {
	length := y-x+1 
	fibSlice := make([]uint64, length)
	j := 0
	for i:=x; i<=y; i++ {
		val, err := cache.GetKey(fmt.Sprintf("%d", i))
		if err == nil {
			fibSlice[j] = val
		}else{
			res := countFibonacci(i)
			fibSlice[j] = res
			cache.SetKey(fmt.Sprintf("%d", i), res, 0)
		}
		j++
	}
	return fibSlice
